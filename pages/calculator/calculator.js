...
Page({
  ...
  calculate: function (str) {
    ...
    for (var i = 0; i < addArray.length; i++) {
      if (addArray[i].indexOf('-') == -1) {
        ...
        var subSum = 0;
        if (subArray[0].indexOf('×') != -1 || subArray[0].indexOf('÷') != -1)
          subSum = this.calculateMutDiv(subArray[0]);
  ...
  clickButton: function (event) {
    if (this.data.result == 0) {
      if (event.target.id == 'back' || event.target.id == 'C' || event.target.id == 'pow' || event.target.id == 'transfer' || event.target.id == '+' || event.target.id == '×' || event.target.id == '÷' || event.target.id == '=') return;
